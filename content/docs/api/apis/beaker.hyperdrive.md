...
You can use the API by instantiating Hyperdrive instances using [.drive()](#beakerhyperdrivedriveurl) or by using the global methods. The global methods can accept paths or URLs. If you pass a path into a global method, the current drive will be used as the target. For example, if you were on a drive `hyper://foobar/`, the following three would be equivalent:
...
* **url** String. The URL of the hyperdrive to query. (Not required on `Hyperdrive` objects returned by [beaker.hyperdrive.drive()](#beakerhyperdrivedriveurl).)
* **opts** Object.
...
var commentsOnBeaker = await beaker.hyperdrive.query({
  drive: 'hyper://1234..ef',
  path: '/comments/*',
  metadata: {href: 'https://nomad.dev'}
})
```

### beaker.hyperdrive.diff(url, other\[, opts\])
...
* **opts** String\|Object. If a string, acts as the encoding parameter.
  ...
  * **metadata** Object. The metadata to write to the file.
  * **timeout** Number (ms). How long to wait for the operation to complete before throwing a timeout error. Defaults to 60000.
* Returns **Promise&lt;Void&gt;**
...
Create a mount on the drive to some other drive. (Mounts are like symlinks that work across hyperdrives.)
