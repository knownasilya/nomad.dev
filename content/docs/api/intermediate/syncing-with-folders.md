...
Sometimes you want to work on a Hyperdrive using files that are outside of Nomad. This is common when you're using your own source-control, like Git, or when you just want to use an editor outside of Nomad.
...
You can choose to skip files by expanding the "Skip items matching these rules" control. This control is similar to ".gitignore" files and supports using glob stars ("*"). Skipping files is useful for avoiding accidental imports (like your .git or node_modules folders) and for preserving data created by nomad (like the index.json manifest).
...
Autosync only stays active as long as Nomad is running, so if you restart Nomad, you'll need to turn it back on.
