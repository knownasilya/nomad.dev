...
- **Nomad mobile** (the Bare/React-Native companion) honors `/.ui/ui.html` at the **drive root** for now; deeper paths resolve to their own files, so content drives degrade to static rendering. **Caveat:** the mobile WebView renders a static, inlined snapshot and currently has **no `nomad` global**, so Frontends that call `nomad.*` (most app templates) render blank there. Plain static Frontends work; `nomad.*` apps remain desktop-only until a mobile `nomad` bridge exists.
...
<script>
  async function main() {
    ...
    if (pathname.endsWith('.html')) {
      main.innerHTML = await nomad.fs.readFile(pathname)
