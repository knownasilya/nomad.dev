...
- **Each post is a directory** — `/posts/<YYYY-MM-DD-slug>/` containing a `post.json` ([`walled.garden/post`](/docs/api/developers/walled-garden-schemas/#post)) and a `post.md` body. Co-locate images in the same folder and reference them with relative paths.
...
- **Posts are URL-addressable**: `hyper://<blog>/posts/<slug>/` is the themed permalink. The post directory has no index file (the body is `post.md`), so that navigation misses and the `fallback` shell renders the post; the raw body stays directly readable at `/posts/<slug>/post.md` on any runtime.
